"""Step 09_N: five-class Soft-label multi-channel benchmark without patient isolation."""
...
def validate_output_root(output_root: Path) -> None:
    if output_root.name != STEP_NAME or output_root.parent.name != "non_isolated":
        raise ValueError(
            "output-root must end with non_isolated/step_09_N; "
...
def reference_split_matches(folded: pd.DataFrame, reference_split: str | Path) -> tuple[bool, str]:
    ...
    if current_map != reference_map:
        return False, "step 09 N fold assignments differ from the fixed reference split"
...
def make_figures(
    ...
    for metric, title, ylabel, formatter, filename in (
        ("accuracy", "step 09 N Accuracy", "accuracy", lambda value: f"{value:.2%}", "accuracy.png"),
        ("macro_f1", "step 09 N Macro-F1", "macro_f1", lambda value: f"{value:.4f}", "macro_f1.png"),
        ("mae", "step 09 N MAE", "mae", lambda value: f"{value:.4f}", "mae.png"),
    ):
    ...
    axis.set_title("step 09 N Per-class F1", loc="left", fontsize=17, pad=16, fontweight="bold")
    axis.set_ylabel("F1", fontsize=13)
    axis.set_xticks(x, [f"Class {class_index}" for class_index in range(NUM_CLASSES)], fontsize=12)
    ...
    for axis, modalities in zip(axes, COMBINATIONS):
        ...
        axis.set_title(combination_axis_label(name), fontsize=13, pad=10, fontweight="bold")
        class_labels = [f"Class {class_index}" for class_index in range(NUM_CLASSES)]
        axis.set_xticks(range(NUM_CLASSES), class_labels, fontsize=9, rotation=30, ha="right")
        axis.set_yticks(range(NUM_CLASSES), class_labels, fontsize=9)
        axis.set_xlabel("Predicted", fontsize=10, labelpad=10)
        axis.set_ylabel("True", fontsize=10, labelpad=10)
        axis.set_aspect("equal")
    for axis in axes[len(COMBINATIONS):]:
        axis.axis("off")
    figure.suptitle("step 09 N Normalized Confusion Matrices", fontsize=20, fontweight="bold")
    figure.subplots_adjust(left=0.06, right=0.90, bottom=0.08, top=0.88, wspace=0.38, hspace=0.38)
    if image is not None:
        figure.colorbar(image, ax=axes.tolist(), shrink=0.82, pad=0.03, label="row proportion")
...
<h1>Step 09_N Five-class Soft-label Multi-channel Benchmark</h1>
...
<section><h2>Figures</h2>{images}</section>
    ...
    (output_root / "report_step_09_N.html").write_text(report, encoding="utf-8")
...
def main() -> None:
    ...
    if args.mode == "train":
        stale_markers = [
            output_root / "metrics_step_09_N.json",
            output_root / "metrics_step_09_N.csv",
            output_root / "report_step_09_N.html",
        ]
        existing_runs = [
            path
        ...
        if any(path.exists() for path in stale_markers) or existing_runs:
            raise RuntimeError("step 09 N contains prior formal results; refusing to mix runs")
    ...
    write_json(output_root / "config_step_09_N.json", config)
    folded[["sample_id", "fold"]].to_csv(output_root / "folds_step_09_N.csv", index=False)
    ...
    for modalities in COMBINATIONS:
        ...
        for training_seed in TRAINING_SEEDS:
            for fold, train_raw, eval_raw in iter_folds(folded, patient_isolation=False):
                ...
                rows.append(row)
                write_json(output_root / "metrics_step_09_N.json", {"fold_results": rows})
                save_csv(output_root / "metrics_step_09_N.csv", rows)
                print(
    ...
    save_csv(output_root / "metrics_step_09_N.csv", rows)
    write_json(output_root / "seed_summaries_step_09_N.json", seed_summaries)
    write_json(output_root / "final_summary_step_09_N.json", final_summaries)
    ...
    print("STEP_09_N_COMPLETE", flush=True)
