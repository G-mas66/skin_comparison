"""Step 07_N: five-class DeepLIFT and channel perturbation for Step 06_N models."""
...
STEP3_DIR = PROJECT_ROOT / "non_isolated" / "step_06_N"
...
from step_06_N import (  # noqa: E402
    BATCH_SIZE,
    BinaryTargetDataset,
    FOLD_SEED,
    NUM_CLASSES,
...
def validate_output_root(output_root: Path) -> None:
    if output_root.name != STEP_NAME or output_root.parent.name != "non_isolated":
        raise ValueError(
            "output-root must end with non_isolated/step_07_N; "
...
def validate_step3_artifacts(
    ...
    errors: list[str] = []
    config_path = step3_root / "config_step_06_N.json"
    split_path = step3_root / "folds_step_06_N.csv"
    if not config_path.is_file():
        errors.append(f"missing step 06 N config: {config_path}")
    else:
        ...
        for key, expected in expected_config.items():
            if config.get(key) != expected:
                errors.append(
                    f"step 06 N config mismatch for {key}: "
                    f"expected {expected!r}, found {config.get(key)!r}"
                )
    if not split_path.is_file():
        errors.append(f"missing step 06 N fixed split: {split_path}")
    else:
        ...
        if current_map != saved_map:
            errors.append("step 06 N fixed split differs from the validated split")
    ...
    if errors:
        raise RuntimeError("step 06 N artifact checks failed: " + "; ".join(errors))
    return report
...
def run_smoke(
    ...
    if tuple(images.shape[1:]) != expected_shape:
        raise RuntimeError(f"Step07_N smoke input shape is invalid: {images.shape}")
    if tuple(attributions.shape) != tuple(images.shape):
        raise RuntimeError(
            f"Step07_N smoke attribution shape is invalid: {attributions.shape}"
        )
...
def save_figures(
    ...
    axis.set_title("step 07 N DeepLIFT modality importance", loc="left", pad=14, fontweight="bold")
    ...
    for metric, title, filename in (
        ...
        axis.set_title(f"step 07 N {title}", loc="left", pad=14, fontweight="bold")
    ...
    for name, modality_maps in spatial_sums.items():
        ...
        figure.suptitle(f"step 07 N mean absolute DeepLIFT map: {name}")
        figure.tight_layout()
...
<html lang="zh-CN"><head><meta charset="utf-8"><title>step_07_N_report</title>
<style>{REPORT_CSS}</style></head><body>
<h1>Step 07_N 五分类 DeepLIFT + 通道扰动</h1>
<p>本次分析针对患者不隔离路线（Patient Isolation = False），包含 <code>M + MR</code>、<code>M + MB + MR</code> 和全通道三个经确认的通道组合；每种组合完整分析 3 个 Training Seed × 5 个 Fold，共 15 个 step 06 N Epoch-50 模型，未按评估性能选择代表模型。</p>
...
{model_table}
    ...
    (output_root / "report_step_07_N.html").write_text(report, encoding="utf-8")
...
def main() -> None:
    ...
    if args.mode == "train":
        stale_markers = [
            output_root / "metrics_step_07_N.json",
            output_root / "report_step_07_N.html",
        ]
        if any(path.exists() for path in stale_markers):
            raise RuntimeError("step 07 N contains prior formal results; refusing to mix runs")
    ...
    config = {
        ...
        "fold_seed": FOLD_SEED,
        "training_seeds": list(TRAINING_SEEDS),
        "evaluation": "each step 06 N model uses its own held-out Fold",
        "deeplift": {
    ...
    write_json(output_root / "config_step_07_N.json", config)
    folded[["sample_id", "fold"]].to_csv(output_root / "folds_step_07_N.csv", index=False)
        ...
        for modalities in COMBINATIONS:
            ...
            for seed in TRAINING_SEEDS:
                for fold, _train_raw, eval_raw in iter_folds(
                    ...
                    save_csv(output_root / "model_results_step_07_N.csv", model_rows)
                    save_csv(
                        output_root / "channel_perturbation_step_07_N.csv",
                        perturbation_rows,
                    )
                    message = (
    ...
    save_csv(output_root / "model_results_step_07_N.csv", model_rows)
    save_csv(output_root / "deeplift_summary_step_07_N.csv", deeplift_summary)
    save_csv(output_root / "channel_perturbation_step_07_N.csv", perturbation_rows)
    write_json(output_root / "deeplift_summary_step_07_N.json", deeplift_summary)
    write_json(output_root / "channel_perturbation_step_07_N.json", perturbation_summary)
    ...
    print("STEP_07_N_COMPLETE", flush=True)
