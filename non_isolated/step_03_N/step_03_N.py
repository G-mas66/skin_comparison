"""Step 03_N: binary multi-modality benchmark without patient isolation."""
...
def validate_output_root(output_root: Path) -> None:
    if output_root.name != STEP_NAME or output_root.parent.name != "non_isolated":
        raise ValueError(
            "output-root must end with non_isolated/step_03_N; "
...
def reference_split_matches(folded: pd.DataFrame, reference_split: str | Path) -> tuple[bool, str]:
    ...
    if current_map != reference_map:
        return False, "Step 03_N fold assignments differ from Step 02_N fixed split"
...
def make_figures(
    ...
    for metric, title, ylabel, formatter, filename in (
        ("accuracy", "Step 03_N Accuracy", "accuracy", lambda value: f"{value:.2%}", "accuracy.png"),
        ("macro_f1", "Step 03_N Macro-F1", "macro_f1", lambda value: f"{value:.4f}", "macro_f1.png"),
        ("mae", "Step 03_N MAE", "mae", lambda value: f"{value:.4f}", "mae.png"),
    ):
    ...
    axis.set_title("Step 03_N Per-class F1", loc="left", fontsize=17, pad=16, fontweight="bold")
    axis.set_ylabel("F1", fontsize=13)
    axis.set_xticks(x, ["Class 0", "Class 1"], fontsize=12)
    ...
    figure, axes = plt.subplots(2, 3, figsize=(14.5, 8.8), squeeze=False)
    axes = axes.ravel()
    image = None
    for axis, modalities in zip(axes, COMBINATIONS):
        ...
        for i in range(NUM_CLASSES):
            for j in range(NUM_CLASSES):
                ...
                axis.text(j, i, f"{counts[i][j]}\n{normalized[i][j]:.1%}", ha="center", va="center", color=color, fontsize=10, fontweight="bold")
        axis.set_title(combination_axis_label(name), fontsize=12, pad=8, fontweight="bold")
        axis.set_xticks([0, 1], ["Class 0", "Class 1"], fontsize=8)
        axis.set_yticks([0, 1], ["Class 0", "Class 1"], fontsize=8)
        axis.set_xlabel("Predicted", fontsize=9)
        axis.set_ylabel("True", fontsize=9)
    for axis in axes[len(COMBINATIONS):]:
        axis.axis("off")
    figure.suptitle("Step 03_N Normalized Confusion Matrices", fontsize=20, y=0.98, fontweight="bold")
        ...
        fold_rows.append(
            {
                ...
                "mae": row["mae"],
                "confusion_matrix": f"[{matrix[0][0]}, {matrix[0][1]}] [{matrix[1][0]}, {matrix[1][1]}]",
            }
        )
    images = "".join(
...
<h1>Step 03_N Binary Multi-modality Benchmark</h1>
<p>Patient Isolation = False；样本级五折交叉验证；Binary labels 0/1/2 → Class 0，3/4 → Class 1；Input 384×384；ResNet50 ImageNet pretrained；Batch Size 32；Epoch 50；AdamW；Learning Rate 1e-4；Weight Decay 1e-4；无 Scheduler、无 Early Stopping。</p>
<p class="note">正式完成度：75/75 new multi-channel runs。Step 02_N 的 M 结果作为既有 Baseline；本 Step 只改变输入通道组合。每个组合包含 3 个 Training Seed × 5 个 Fold。</p>
...
<section><h2>Figures</h2>{images}</section>
    ...
    (output_root / "report_step_03_N.html").write_text(report, encoding="utf-8")
...
def main() -> None:
    ...
    if args.mode == "train":
        stale_markers = [
            output_root / "metrics_step_03_N.json",
            output_root / "metrics_step_03_N.csv",
            output_root / "report_step_03_N.html",
        ]
        existing_runs = [
            path
        ...
        if any(path.exists() for path in stale_markers) or existing_runs:
            raise RuntimeError("Step 03_N contains prior formal results; refusing to mix runs")
    ...
    config = {
        ...
        "baseline_reference": "non_isolated/step_02_N M",
        "input_size": [INPUT_SIZE, INPUT_SIZE],
        "model": MODEL_NAME,
        "pretrained": True,
        "num_classes": NUM_CLASSES,
    ...
    write_json(output_root / "config_step_03_N.json", config)
    folded[["sample_id", "fold"]].to_csv(output_root / "folds_step_03_N.csv", index=False)
    ...
    for modalities in COMBINATIONS:
        ...
        for training_seed in TRAINING_SEEDS:
            for fold, train_raw, eval_raw in iter_folds(folded, patient_isolation=False):
                ...
                rows.append(row)
                write_json(output_root / "metrics_step_03_N.json", {"fold_results": rows})
                save_csv(output_root / "metrics_step_03_N.csv", rows)
                print(
    ...
    save_csv(output_root / "metrics_step_03_N.csv", rows)
    write_json(output_root / "seed_summaries_step_03_N.json", seed_summaries)
    write_json(output_root / "final_summary_step_03_N.json", final_summaries)
    baseline_path = output_root.parent / "step_02_N" / "final_summary_step_02_N.json"
    ...
    print("STEP_03_N_COMPLETE", flush=True)
