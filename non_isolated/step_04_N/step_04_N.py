"""Step 04_N: DeepLIFT and channel perturbation for selected Step 03_N models."""
...
STEP3_DIR = PROJECT_ROOT / "non_isolated" / "step_03_N"
...
from step_03_N import (  # noqa: E402
    BATCH_SIZE,
    BinaryTargetDataset,
    FOLD_SEED,
    NUM_CLASSES,
...
def validate_output_root(output_root: Path) -> None:
    if output_root.name != STEP_NAME or output_root.parent.name != "non_isolated":
        raise ValueError(
            "output-root must end with non_isolated/step_04_N; "
...
def validate_step3_artifacts(
    ...
    errors: list[str] = []
    config_path = step3_root / "config_step_03_N.json"
    split_path = step3_root / "folds_step_03_N.csv"
    if not config_path.is_file():
        errors.append(f"missing Step 03_N config: {config_path}")
    else:
        ...
        for key, expected in expected_config.items():
            if config.get(key) != expected:
                errors.append(
                    f"Step 03_N config mismatch for {key}: "
                    f"expected {expected!r}, found {config.get(key)!r}"
                )
    if not split_path.is_file():
        errors.append(f"missing Step 03_N fixed split: {split_path}")
    else:
        ...
        if current_map != saved_map:
            errors.append("Step 03_N fixed split differs from the validated split")
    ...
    if errors:
        raise RuntimeError("Step 03_N artifact checks failed: " + "; ".join(errors))
    return report
...
def analyze_model(
    ...
    model_row = {
        ...
        "training_seed": seed,
        "evaluation_count": len(eval_frame),
        "checkpoint": str(checkpoint_path.relative_to(STEP3_DIR)),
...
def save_figures(
    ...
    axis.set_title("Step 04_N DeepLIFT modality importance", loc="left", pad=14, fontweight="bold")
    ...
    for metric, title, filename in (
        ...
        axis.set_title(f"Step 04_N {title}", loc="left", pad=14, fontweight="bold")
    ...
    for name, modality_maps in spatial_sums.items():
        ...
        figure.suptitle(f"Step 04_N mean absolute DeepLIFT map: {name}")
        figure.tight_layout()
...
<html lang="zh-CN"><head><meta charset="utf-8"><title>step_04_N_report</title>
<style>{REPORT_CSS}</style></head><body>
<h1>Step 04_N 二分类 DeepLIFT + 通道扰动</h1>
<p>本次分析针对患者不隔离路线（Patient Isolation = False），包含 <code>M + MR</code>、<code>M + MB + MR</code> 和全通道三个经确认的通道组合；每种组合完整分析 3 个 Training Seed × 5 个 Fold，共 15 个 Step 03_N Epoch-50 模型，未按评估性能选择代表模型。</p>
...
{model_table}
    ...
    (output_root / "report_step_04_N.html").write_text(report, encoding="utf-8")
...
def main() -> None:
    ...
    if args.mode == "train":
        stale_markers = [
            output_root / "metrics_step_04_N.json",
            output_root / "report_step_04_N.html",
        ]
        if any(path.exists() for path in stale_markers):
            raise RuntimeError("Step 04_N contains prior formal results; refusing to mix runs")
    ...
    config = {
        ...
        "fold_seed": FOLD_SEED,
        "training_seeds": list(TRAINING_SEEDS),
        "evaluation": "each Step 03_N model uses its own held-out Fold",
        "deeplift": {
    ...
    write_json(output_root / "config_step_04_N.json", config)
    folded[["sample_id", "fold"]].to_csv(output_root / "folds_step_04_N.csv", index=False)
        ...
        for modalities in COMBINATIONS:
            ...
            for seed in TRAINING_SEEDS:
                for fold, _train_raw, eval_raw in iter_folds(
                    ...
                    save_csv(output_root / "model_results_step_04_N.csv", model_rows)
                    save_csv(
                        output_root / "channel_perturbation_step_04_N.csv",
                        perturbation_rows,
                    )
                    message = (
    ...
    save_csv(output_root / "model_results_step_04_N.csv", model_rows)
    save_csv(output_root / "deeplift_summary_step_04_N.csv", deeplift_summary)
    save_csv(output_root / "channel_perturbation_step_04_N.csv", perturbation_rows)
    write_json(output_root / "deeplift_summary_step_04_N.json", deeplift_summary)
    write_json(output_root / "channel_perturbation_step_04_N.json", perturbation_summary)
    ...
    print("STEP_04_N_COMPLETE", flush=True)
