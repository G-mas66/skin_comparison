...
def _assign_patient_folds(frame: pd.DataFrame, n_folds: int, seed: int) -> dict[str, int]:
    ...
    def dominant_label(group: pd.DataFrame) -> int:
        counts = group["label"].value_counts()
        return min(
            (int(label) for label in counts.index),
            key=lambda label: (-int(counts[label]), label),
        )

    # Deterministic stratified least-loaded assignment: patient blocks are
    # bucketed by dominant label, each bucket is ordered largest-block-first
    # (stable, so the seeded shuffle breaks ties), and every block goes to the
    # fold that currently holds the fewest samples overall, then the fewest
    # samples of that label, then the lowest fold index. Round-robin balance
    # avoids the degenerate local optima of the previous distance-summing
    # greedy (which produced 396/396/202/1/1 folds).
    for label in labels:
        bucket = [group for group in groups if dominant_label(group[1]) == label]
        bucket.sort(key=lambda item: len(item[1]), reverse=True)
        for _, group in bucket:
            selected_fold = min(
                range(n_folds),
                key=lambda fold: (fold_sizes[fold], fold_labels[fold][label], fold),
            )
            fold_sizes[selected_fold] += len(group)
            for lab in labels:
                fold_labels[selected_fold][lab] += int((group["label"] == lab).sum())
