"""Step 04_I - binary DeepLIFT + channel perturbation under patient isolation.

Representative schemes (explicitly user-confirmed 2026-09-15, before any
Step 04 analysis ran): M+MB+MR (best and most stable in Step 03_I),
M+MR (runner-up, highest AUC), and all five channels (full-input reference).
Selection basis and user confirmation are recorded in config and notes.

Protocol-safe analysis design:
  - NO checkpoint selection: for every scheme, ALL 15 trained models
    (Training Seeds 42/3407/2026 x Folds 1-5) are analysed, each on its own
    evaluation fold, and aggregated as Mean +/- Std across the 15 runs -
    mirroring the training benchmark structure exactly.
  - Channel perturbation (identical for every scheme): one channel at a
    time is replaced with black pixels (the same fill used by the black
    center padding, so the perturbation stays in-distribution) and the fold
    is re-evaluated. Metrics: Accuracy / Macro-F1 / AUC deltas vs the
    unperturbed baseline of the same model on the same fold.
  - DeepLIFT quantitative: attribution w.r.t. each sample's predicted class
    against an all-black reference input passed through the identical
    preprocessing; channel importance = share of summed |attribution| per
    channel, averaged over all evaluation samples of the fold.
  - DeepLIFT visual examples follow a pre-specified rule: the seed 42 / fold
    1 model, first two correctly classified evaluation samples per class in
    dataset order (no cherry-picking).

Weights are loaded read-only from step_03_I fold checkpoints.
"""
...
STEP_03_DIR = STEP_DIR.parent / "step_03_I"
...
METRIC_KEYS = ("accuracy", "macro_f1", "auc")

# 中文版报告说明（渲染进 HTML 报告；metrics JSON 保留英文原始记录作为数据档案）。
REPORT_NOTES_ZH = [
    "代表性方案在 Step 04 任何分析运行之前由用户明确确认（2026-09-15）锁定："
    "M+MB+MR、M+MR 与全五通道。选择依据（Step 03_I 排名）与确认事实记录于 "
    "config_step_04_I.json。",
    ...
    "Padding 使用相同的填充色，保持扰动在分布内。不重训、不改划分。",
    "DeepLIFT：captum 实现；参考输入为经完全相同预处理的纯黑图像；"
    "归因目标为每个样本的预测类别；通道重要性 = 每样本内该通道 |attribution| 总和的"
    "占比，按 fold 模型平均后在 15 次 run 上汇总。",
    "可视化示例遵循预定规则（seed 42 / fold 1 模型、数据集顺序中每类前 2 个预测正确"
    "的评估样本），仅作定性展示；定量结论以 15 次 run 的完整聚合为准。",
    "【补充项】（用户要求，2026-09-15，经对比此前 rosacea_step4 报告格式后）：新增 "
    "(a) 中心 vs 外周的定量空间归因划分（中心框 = 每边中央 50%，即 384 图上的 "
    "192×192）、(b) 含 Sensitivity/Specificity 的扰动后绝对性能表、(c) 本报告的数据"
    "完整性章节。仅分析层面的补充；无重训、无划分改动、每方案仍为相同 15 个模型。",
]
...
def evaluate(model: nn.Module, loader: DataLoader, device,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            logits = model(images)
            probs.extend(float(p) for p in torch.softmax(logits, 1)[:, 1].cpu())
            preds.extend(int(p) for p in logits.argmax(1).cpu())
            targets.extend(binary_target(int(t)) for t in batch["target"])
    targets_array = np.array(targets)
    preds_array = np.array(preds)
    probs_array = np.array(probs)
    tp = int(((targets_array == 1) & (preds_array == 1)).sum())
    fn = int(((targets_array == 1) & (preds_array == 0)).sum())
    tn = int(((targets_array == 0) & (preds_array == 0)).sum())
    fp = int(((targets_array == 0) & (preds_array == 1)).sum())
    sensitivity = tp / (tp + fn) if (tp + fn) else float("nan")
    specificity = tn / (tn + fp) if (tn + fp) else float("nan")
    return {
        "accuracy": float(accuracy_score(targets_array, preds_array)),
        "macro_f1": float(f1_score(targets_array, preds_array, average="macro")),
...
def render_examples(model: nn.Module, dataset: SkinDataset, device,
    ...
    for index in range(len(dataset)):
        item = dataset[index]
        target = binary_target(item["target"])
        with torch.no_grad():
...
def finalize_outputs(payload: dict, figures_dir: Path) -> None:
    ...
    for record in records:
        row = {
            ...
            "baseline_auc": record["baseline"]["auc"],
            "baseline_sensitivity": record["baseline"]["sensitivity"],
            "baseline_specificity": record["baseline"]["specificity"],
        }
        for channel in record["channels"]:
            for metric in (*METRIC_KEYS, "sensitivity", "specificity"):
    ...
    for metric, label in (
        ...
        ax.set_ylabel(label)
        ax.set_title(
            f"Step 04_I {TASK} | channel perturbation impact | {label} "
    ...
    ax.set_title(
        "Step 04_I binary | per-channel DeepLIFT importance share "
        "(mean ± std over 15 runs per scheme)"
    )
    ax.legend(fontsize=9)
    fig.tight_layout()
    ...
    ax.set_title(
        "Step 04_I binary | DeepLIFT center-vs-periphery split "
    ...
    for scheme_key, modalities in SCHEMES:
        ...
        baseline_rows.append(
            ...
            + "".join(
                ...
                for m in ("accuracy", "macro_f1", "auc", "sensitivity", "specificity")
            )
            + "</tr>"
        )
    sections.append(
        "<h2>未扰动基线性能（15 个模型各自在其评估折，Mean ± Std）</h2>"
        "<table><tr><th>方案</th><th>Accuracy</th><th>Macro-F1</th><th>AUC</th>"
    ...
    sections.append(
        ...
        "<table><tr><th>方案</th><th>扰动</th><th>Accuracy</th><th>Macro-F1</th><th>AUC</th>"
        "<th>Sensitivity</th><th>Specificity</th></tr>"
...
<body>
<h1>{STEP_NAME}：二分类 DeepLIFT + 通道扰动（Patient Isolation = True）</h1>
...
def main() -> None:
    ...
    config = {
        ...
        "benchmark": "binary DeepLIFT + channel perturbation (interpretability)",
        "task": TASK,
        "num_classes": NUM_CLASSES,
        ...
        "representative_schemes": [
            ...
            for (key, modalities), basis in zip(
                SCHEMES,
                (
                    "best macro-F1 and lowest seed variance in Step 03_I",
                    "runner-up macro-F1, highest AUC in Step 03_I",
        ...
        for fold_number in range(1, 6):
            ...
            for seed in TRAINING_SEEDS:
                ...
                if key in fold_metrics:
                    ...
                    continue
                checkpoint = (
                    STEP_03_DIR / f"fold_{fold_number:02d}" / key / "checkpoints" / "last.pth"
                )
    ...
    all_metrics = (*METRIC_KEYS, "sensitivity", "specificity")
    for scheme_key, modalities in SCHEMES:
    ...
    notes = [
        ...
        "confirmation are recorded in config_step_04_I.json.",
        ...
        "performance tables including sensitivity/specificity, and (c) the "
    ...
    # ----- figures -----
    scheme_colors = {"M_MB_MR": "#4C72B0", "M_MR": "#DD8452", "ALL5": "#55A868"}
    for metric, label in (("macro_f1", "ΔMacro-F1 (drop)"), ("accuracy", "ΔAccuracy (drop)"), ("auc", "ΔAUC (drop)")):
        fig, ax = plt.subplots(figsize=(9, 4))
        x = np.arange(len(ALL_CHANNELS))
        width = 0.25
        for offset, (scheme_key, modalities) in enumerate(SCHEMES):
