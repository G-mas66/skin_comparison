"""Step 08_I - five-class SOFT-LABEL single-channel benchmark.

Label Strategy is the only main variable vs Step 05_I (hard label): training
targets come from the user-specified FIXED soft-label lookup table (see
SOFT_LABEL_TABLE below), applied at runtime by looking up the ORIGINAL
0-4 label; the manifest is never modified. Loss = soft-target cross entropy
-(soft * log_softmax).sum(1).mean(). Evaluation is unchanged: argmax logits
against the original hard labels, so results stay directly comparable with
Step 05. All five channels are retrained (75 runs). Hard-label results are
imported verbatim from Step 05_I for the Hard vs Soft comparison.
"""
...
SCHEME_COLORS = {
    ...
    "MUV": "#8172B3", "soft_M": "#4C72B0", "soft_MB": "#DD8452",
    "soft_MP": "#55A868", "soft_MR": "#C44E52", "soft_MUV": "#8172B3",
}
INFRASTRUCTURE = {
...
REPORT_NOTES_ZH = [
    ...
    "Hard Label 对照结果原样引用 Step 05_I 的 15 条/通道真实记录，"
    ...
    "均为基础设施层面改动，协议超参数零变化。",
    "五个通道全部重新训练（Soft Label 改变了训练目标，无 run 可复用；"
    "5 通道 × 3 Training Seeds × 5 Folds = 75 次）。",
    "重点观察 Soft Label 是否降低相邻等级误判率与 MAE（相邻边界模糊问题）。",
...
def make_figures(context_summaries, combo_summaries, figures_dir) -> list[Path]:
    ...
    schemes = (
        [(f"{k} (Hard)", k, "hard") for k in SINGLE_CHANNELS]
        + [(SCHEME_DISPLAY[k], k, "soft") for k, _ in SCHEMES_TRAINED]
    )
    ...
    for key, label, percent in (
        ...
        ax.set_ylabel(label)
        ax.set_title(
            f"Step 08_I 5-class | White baseline vs multi-channel schemes | {label} "
    ...
    fig.suptitle("Step 08_I 5-class | per-class metrics by scheme (final 3-seed mean ± std)")
    fig.tight_layout()
    for suffix in ("png", "pdf"):
    ...
    fig.suptitle("Step 08_I 5-class | confusion matrices (5 folds x 3 seeds; M reused from Step 05)")
    fig.tight_layout()
    for suffix in ("png", "pdf"):
...
def build_html_report(config, integrity, context_summaries, combo_summaries,
    ...
    all_summaries = {**context_summaries, **combo_summaries}
    schemes = (
        [(f"{k} (Hard)", k, "引用 Step 05") for k in SINGLE_CHANNELS]
    ...
    sections.append("<h2>Soft Label 通道 Fold 级原始结果（本次训练 75 条）</h2>")
    ...
    compare_rows = []
    for channel in SINGLE_CHANNELS:
        hard = context_summaries[channel]["final"]
        soft = combo_summaries[f"soft_{channel}"]["final"]
        compare_rows.append(
            f"<tr><td>{channel}</td>"
...
def main() -> None:
    ...
    config = {
        ...
        "benchmark": "five-class soft-label single-channel (label strategy benchmark vs hard-label Step 05)",
        "task": TASK,
        ...
        "hard_label_comparison_source": (
            "Step 05_I metrics imported verbatim (identical configuration "
            "except hard labels)"
        ),
        "model": MODEL_NAME,
        "pretrained": "ImageNet",
    ...
    if args.smoke:
        smoke_dir = guard_path(logs_dir / "smoke" / "soft_M_fold_01_seed_42")
        ...
        train_dataset = SkinDataset(train_frame, data_root,
                                    modalities=("M",), training=True)
        eval_dataset = SkinDataset(eval_frame, data_root,
                                   modalities=("M",), training=False)
        log("SMOKE TEST: channel M (soft label), fold 1, seed 42, 2 epochs, 5-class")
    ...
    notes = [
        "Label Strategy is the only main variable vs Step 05_I: training "
        "uses the user-specified fixed soft-label table; evaluation is "
        "unchanged (argmax vs original hard labels).",
        "Hard-label single-channel results imported verbatim from Step 05_I "
        "for the Hard vs Soft comparison.",
