"""Step 07_I - five-class DeepLIFT + channel perturbation under patient isolation.

Representative schemes (pre-specified 2026-09-15 before any Step 06/07
result was aggregated, mirroring the Step 04 interpretability route for
cross-task comparability): M+MB+MR, M+MR, and all five channels. The
pre-specification rule and basis are recorded in config and notes.
...
Protocol-safe analysis design:
  ...
  - Channel perturbation (identical for every scheme): one channel at a
    ...
    is re-evaluated. Metrics: Accuracy / Macro-F1 / MAE deltas vs the
  ...
  - DeepLIFT visual examples follow a pre-specified rule: the seed 42 / fold
    1 model, first correctly classified evaluation sample per class (0-4)
    in dataset order (no cherry-picking).

Weights are loaded read-only from step_06_I fold checkpoints.
"""
...
REPORT_NOTES_ZH = [
    "代表性方案于 2026-09-15 在任何 Step 06/07 结果聚合之前预先指定（沿用 Step 04 "
    "解释性路线的三方案：M+MB+MR、M+MR、全五通道），便于二分类与五分类解释性结论"
    "横向对照。预指定规则与依据记录于 config_step_07_I.json。",
    ...
    "权重只读自 Step 06_I 的 fold checkpoints（五分类 Hard Label 多通道模型）。",
]
...
def finalize_outputs(payload: dict, figures_dir: Path) -> None:
    ...
    for metric, label in (
        ...
        ax.set_ylabel(label)
        ax.set_title(
            f"Step 07_I 5-class | channel perturbation impact | {label} "
    ...
    ax.set_title(
        "Step 07_I 5-class | per-channel DeepLIFT importance share "
        "(mean ± std over 15 runs per scheme)"
    )
    ax.legend(fontsize=9)
    fig.tight_layout()
    ...
    ax.set_title(
        "Step 07_I 5-class | DeepLIFT center-vs-periphery split "
...
<body>
<h1>{STEP_NAME}：五分类 DeepLIFT + 通道扰动（Patient Isolation = True）</h1>
...
def main() -> None:
    ...
    config = {
        ...
        "benchmark": "five-class DeepLIFT + channel perturbation (interpretability)",
        "task": TASK,
        "num_classes": NUM_CLASSES,
        ...
        for fold_number in range(1, 6):
            ...
            for seed in TRAINING_SEEDS:
                ...
                if key in fold_metrics:
                    ...
                    continue
                checkpoint = (
                    STEP_06_DIR / f"fold_{fold_number:02d}" / key / "checkpoints" / "last.pth"
                )
    ...
    notes = [
        ...
        "confirmation are recorded in config_step_07_I.json.",
