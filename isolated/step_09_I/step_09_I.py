"""Step 09_I - five-class SOFT-LABEL multi-channel combinations.

Same pre-registered combination list as Step 06 (mirroring the user-approved
Step 03 list): M+MR, M+MB+MR, all five channels, M+MB, MB+MR - trained with
the user-specified FIXED soft-label table (see SOFT_LABEL_TABLE) and the
soft-target cross entropy. Label Strategy is the only main variable vs
Step 06 (hard-label combinations); evaluation is unchanged (argmax vs the
original hard labels). Soft single-channel results are imported verbatim
from Step 08_I as context/baseline; hard-label combination results from
Step 06_I are imported for the Hard vs Soft comparison.
"""
...
TASK = "5class"
TASK_DISPLAY = "五分类（0-4 严重程度，Soft Label 固定查找表）"
NUM_CLASSES = 5
...
STEP_08_DIR = STEP_DIR.parent / "step_08_I"
STEP_06_DIR = STEP_DIR.parent / "step_06_I"

# User-specified fixed soft-label lookup table (2026-09-15). Applied at
# runtime by looking up the ORIGINAL label; the manifest is never modified.
SOFT_LABEL_TABLE: tuple[tuple[float, ...], ...] = (
...
SCHEME_DISPLAY = {
    "M_MR": "M+MR (Soft)",
    "M_MB_MR": "M+MB+MR (Soft)",
    "ALL5": "M+MB+MP+MR+MUV (Soft)",
    "M_MB": "M+MB (Soft)",
    "MB_MR": "MB+MR (Soft)",
}
REFERENCE_CHANNEL = "M"
...
INFRASTRUCTURE = {
    ...
    "note": (
        "user-approved 2026-09-15, effective from Step 06 (see the Step 06 "
        "FD-limit incident note); infrastructure only, no protocol "
...
REPORT_NOTES_ZH = [
    "本 Step 为五分类 Soft Label 多通道实验：Label Strategy 是相对 Step 06 "
    "（Hard Label 单通道）的唯一主要变量，其余配置（任务、分辨率、五折划分、"
    "Fold Seed 42、Training Seeds、ResNet50 ImageNet、AdamW 1e-4/1e-4、"
    "Batch Size 32、50 Epoch、增强与预处理）与 Step 06 完全一致。",
    "Soft Label 使用用户 2026-09-15 指定的固定查找表（硬编码于脚本，运行时按"
    "原始标签查表，不修改 manifest 的原始 0-4 标签）："
    "0→[0.90,0.10,0,0,0]，1→[0.10,0.80,0.10,0,0]，2→[0,0.10,0.80,0.10,0]，"
    "3→[0,0,0.10,0.80,0.10]，4→[0,0,0,0.10,0.90]。Loss 为软目标交叉熵 "
    "-Σ(soft·log_softmax)；评估仍用 argmax 对原始硬标签。",
    "Soft 单通道对照原样引用 Step 08_I，Hard Label 多通道对照原样引用 Step 06_I，"
    "用于 Hard vs Soft 直接比较（见对比表与图表）。",
    "【经批准的提速优化】（用户，2026-09-15，自 Step 06 生效）：persistent_workers、"
    "每折数据集复用、cudnn.benchmark + channels_last、RLIMIT_NOFILE 提升。"
    "均为基础设施层面改动，协议超参数零变化。",
    "多通道组合沿用 Step 06 预注册清单（与 Step 03 一致，任何 Soft Label 多通道训练前固定）；"
    "组合全部重新训练（5 组合 × 3 Training Seeds × 5 Folds = 75 次）。",
    "重点观察 Soft Label 多通道相对 Hard Label 多通道（Step 06）的性能变化。",
...
def train_one_run(
    ...
    soft_table = torch.tensor(SOFT_LABEL_TABLE, dtype=torch.float32, device=device)

    def soft_cross_entropy(logits, targets):
        soft_targets = soft_table[targets]
    ...
    with open(train_log_path, "w", encoding="utf-8") as train_log:
        train_log.write(
            ...
            f"loss=soft-target CE (fixed table) scheduler=None early_stopping=False task=5class\n"
        )
        train_log.flush()
...
def load_single_channel_context():
    payload = json.loads(
        (STEP_08_DIR / "metrics_step_08_I.json").read_text(encoding="utf-8")
    )
    soft = payload["combo_summaries"]  # keys: soft_M, soft_MB, ...
    summaries = {
        channel: soft[f"soft_{channel}"] for channel in SINGLE_CHANNELS
    }
    all_runs = []
    for channel in SINGLE_CHANNELS:
        for seed in TRAINING_SEEDS:
...
def make_figures(context_summaries, combo_summaries, figures_dir) -> list[Path]:
    ...
    schemes = (
        [(f"{k} (Soft)", k, "soft single") for k in SINGLE_CHANNELS]
        + [(SCHEME_DISPLAY[k], k, "soft combo") for k, _ in SCHEMES_TRAINED]
    )
    ...
    for key, label, percent in (
        ...
        ax.set_ylabel(label)
        ax.set_title(
            f"Step 09_I 5-class | White baseline vs multi-channel schemes | {label} "
            "(bar = mean over 5 folds; soft singles reused from Step 08)"
        )
    ...
    fig.suptitle("Step 09_I 5-class | per-class metrics by scheme (final 3-seed mean ± std)")
    fig.tight_layout()
    for suffix in ("png", "pdf"):
    ...
    matrix_schemes = [k for k, _ in SCHEMES_TRAINED]
    ...
    fig.suptitle("Step 09_I 5-class | confusion matrices (5 folds x 3 seeds; soft singles reused from Step 08)")
    fig.tight_layout()
    for suffix in ("png", "pdf"):
...
def build_html_report(config, integrity, context_summaries, combo_summaries,
    ...
    all_summaries = {**context_summaries, **combo_summaries}
    schemes = (
        [(f"{k} (Soft)", k, "引用 Step 08") for k in SINGLE_CHANNELS]
    ...
    sections.append("<h2>Soft Label 多通道方案 Fold 级原始结果（本次训练 75 条）</h2>")
    ...
    hard_combo_summaries = json.loads(
        (STEP_06_DIR / "metrics_step_06_I.json").read_text(encoding="utf-8")
    )["combo_summaries"]
    compare_rows = []
    for scheme_key, _ in SCHEMES_TRAINED:
        hard = hard_combo_summaries[scheme_key]["final"]
        soft = combo_summaries[scheme_key]["final"]
        compare_rows.append(
            f"<tr><td>{SCHEME_DISPLAY[scheme_key]}</td>"
            f"<td>{hard['accuracy']['mean'] * 100:.2f}%</td>"
            f"<td>{soft['accuracy']['mean'] * 100:.2f}%"
            f" ({(soft['accuracy']['mean'] - hard['accuracy']['mean']) * 100:+.2f})</td>"
            f"<td>{hard['macro_f1']['mean']:.4f}</td>"
            f"<td>{soft['macro_f1']['mean']:.4f}"
            f" ({(soft['macro_f1']['mean'] - hard['macro_f1']['mean']) * 100:+.2f})</td>"
            f"<td>{hard['mae']['mean']:.4f}</td>"
            f"<td>{soft['mae']['mean']:.4f}"
            f" ({soft['mae']['mean'] - hard['mae']['mean']:+.4f})</td>"
            f"<td>{hard['adjacent_error_rate']['mean']:.4f}</td>"
            f"<td>{soft['adjacent_error_rate']['mean']:.4f}"
            f" ({(soft['adjacent_error_rate']['mean'] - hard['adjacent_error_rate']['mean']) * 100:+.2f})</td>"
            f"</tr>"
        )
    sections.append(
        "<h2>Hard vs Soft Label 对比（括号内 = Soft − Hard）</h2>"
        "<table><tr><th>Channel</th>"
        "<th>Acc (Hard)</th><th>Acc (Soft)</th>"
        "<th>Macro-F1 (Hard)</th><th>Macro-F1 (Soft)</th>"
        "<th>MAE (Hard)</th><th>MAE (Soft)</th>"
        "<th>相邻误判率 (Hard)</th><th>相邻误判率 (Soft)</th></tr>"
...
def main() -> None:
    ...
    config = {
        ...
        "benchmark": "five-class soft-label multi-channel combinations (label strategy benchmark vs hard-label Step 06)",
        "task": TASK,
        "task_display": TASK_DISPLAY,
        "num_classes": NUM_CLASSES,
        "label_strategy": ("soft label: user-specified fixed lookup table applied at runtime from the original 0-4 label; manifest unchanged"),
        "soft_label_table": [list(row) for row in SOFT_LABEL_TABLE],
        "patient_isolation": PATIENT_ISOLATION,
        "fold_seed": FOLD_SEED,
        "training_seeds": list(TRAINING_SEEDS),
        "input_resolution": f"{INPUT_SIZE}x{INPUT_SIZE}",
        "channels_trained": [list(m) for _, m in SCHEMES_TRAINED],
        "hard_label_comparison_source": (
            "Step 06_I combination metrics imported verbatim (identical "
            "configuration except hard labels)"
        ),
        "soft_single_context_source": (
            "Step 08_I metrics imported verbatim (identical soft-label "
            "configuration, single channels)"
        ),
        "schemes_preregistered_note": (
            "same combination list as Step 06 (user-approved Step 03 list), "
            "fixed before any soft-label multi-channel training"
        ),
        "model": MODEL_NAME,
        "pretrained": "ImageNet",
        "optimizer": "AdamW",
        ...
        "loss": "soft-target cross entropy: -(soft * log_softmax(logits)).sum(1).mean()",
    ...
    if args.smoke:
        ...
        log("SMOKE TEST: scheme ALL5 (soft label, 15-channel), fold 1, seed 42, 2 epochs")
    ...
    notes = [
        "Label Strategy is the only main variable vs Step 06_I: training "
        "uses the user-specified fixed soft-label table; evaluation is "
        "unchanged (argmax vs original hard labels).",
        "Combination list pre-registered (same as Step 06 / Step 03). "
        "Soft single-channel results imported from Step 08_I; hard-label "
        "combination results imported from Step 06_I for comparison.",
        ...
        "All schemes use exactly the same 996 samples and fold split as Steps 05-08.",
